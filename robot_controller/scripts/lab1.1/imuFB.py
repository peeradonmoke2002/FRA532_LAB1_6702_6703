...
class KinematicOdometryNode(Node):
    def __init__(self):
        ...
        # 🔄 Odometry Scaling
        self.ODO_SCALE = 0.99  # Empirical scaling factor for odometry correction
        ...
        self.v = 0.0
        self.omega_enc = 0.0  # Raw yaw rate from encoders
        self.omega_imu = 0.0  # Raw yaw rate from IMU
        ...
        self.imu_weight = 0.8  # Weight for IMU in complementary filter
        ...
        self.get_logger().info("✅ Kinematic Odometry Node Started with IMU Feedback!")
    ...
    def joint_state_callback(self, msg: JointState):
        try:
            ...
            if abs(delta_fl) > 1e-4 or abs(delta_fr) > 1e-4:  # Avoid division by zero
                self.omega_enc = (v_fl * np.tan(delta_fl) + v_fr * np.tan(delta_fr)) / self.WHEEL_BASE
            else:
                self.omega_enc = 0.0  # No steering input, no rotation

        except ValueError as e:
            self.get_logger().warn(f"⚠️ Joint name not found: {e}")

    def imu_callback(self, msg: Imu):
        """ Extracts yaw rate (ω) from the IMU message. """
        self.omega_imu = msg.angular_velocity.z  # IMU yaw rate

    def publish_odometry(self):
        # ✅ Fuse Yaw Rate (Complementary Filter)
        self.filtered_omega = (self.imu_weight * self.omega_imu) + ((1 - self.imu_weight) * self.omega_enc)

        # 📍 Update Pose Using Fused Yaw Rate (ω_k)
        ...
        self.get_logger().info(f"📡 Odometry Published → X: {self.x:.3f}, Y: {self.y:.3f}, Theta: {np.degrees(self.theta):.3f}°, v_k: {self.v:.3f} m/s, ω_enc: {self.omega_enc:.3f} rad/s, ω_imu: {self.omega_imu:.3f} rad/s, ω_fused: {self.filtered_omega:.3f} rad/s")
