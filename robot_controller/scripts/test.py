...
class OdometryCalculation(Node):
    def __init__(self):
        ...
        self.create_subscription(JointState, '/joint_states', self.wheel_vel_callback, 10)
        ...
        self.L_REAR_STEER_ANGLE = 0.0 # rad
        self.R_REAR_STEER_ANGLE = 0.0 # rad
        self.L_RX = 0.0 # mm
        self.R_RX = 0.0 # mm
        self.L_RY = - self.track_width / 2 # mm
        self.R_RY = self.track_width / 2 # mm
    ...
    #     """Callback to process cmd_vel (linear and angular velocities)."""
    #     v_curr = msg.linear.x
    #     w_curr = msg.angular.z
    #     cmd_vel = [v_curr, w_curr]
    #     self.turn_sign = np.sign(cmd_vel[1])

    def wheel_vel_callback(self, msg):
        """Callback to process joint states (wheel velocities and steering angles)."""
        if 'rear_left_wheel' in msg.name and 'rear_right_wheel' in msg.name and 'front_left_steering' in msg.name and 'front_right_steering' in msg.name:
            
            left_wheel_index = msg.name.index('rear_left_wheel')
            right_wheel_index = msg.name.index('rear_right_wheel')

            steering_fl_index = msg.name.index('front_left_steering')
            steering_fr_index = msg.name.index('front_right_steering')

            self.v_rl = msg.velocity[left_wheel_index] * self.wheel_radius  # Rear Left Velocity
            self.v_rr = msg.velocity[right_wheel_index] * self.wheel_radius  # Rear Right Velocity
            if self.turn_sign > 0:
                # turn left
                self.L_REAR_STEER_ANGLE = 0.523598767
                self.R_REAR_STEER_ANGLE = 0.523598767
            else:
                # turn right
                self.L_REAR_STEER_ANGLE = - 0.523598767
                self.R_REAR_STEER_ANGLE = - 0.523598767
    ...
    def Odo2Track(self):
        ...
        print('x: ',self.x_curr , 'y: ',self.y_curr, 'z', self.quat[2], 'w', self.quat[3])
    ...
    def OdoYawRate(self):
        ...
        self.w_curr = (self.v_rl - self.v_rr)/(self.R_RY - self.L_RY)
