...
from nav_msgs.msg import Path, Odometry  # Only used for publishing path
...
class StanleyNode(Node):
    def __init__(self):
        ...
        self.create_timer(0.05, self.timer_callback)  # 20 Hz control loop
        ...
        # Storage for the robot state (from Gazebo model_states)
    ...
    def stanley_controller(self):
        ...
        # Extract current state from self.robot_pose
        # Optionally add a small offset if needed.
    ...
    def model_states_callback(self, msg: ModelStates):
        ...
        Extracts the state of the model named "limo".
        """
        try:
            index = msg.name.index("limo")
        except ValueError:
            self.get_logger().error("Limo model not found in Gazebo!")
            return
